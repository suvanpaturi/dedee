...
class Neo4jContentUploader:
    ...
    def process_new_format(self, data):
        ...
        try:
            ...
            # Create vector search indexes if data has embeddings
            if count > 0 and 'embedding' in data['devices'][0]['queries'][0]['query']:
...
def main():
    ...
    if neo4j_uri == "neo4j://localhost:7687" or neo4j_username == "neo4j" or neo4j_password == "password":
        try:
            from dotenv import load_dotenv
            load_dotenv()
            neo4j_uri = os.environ.get("NEO4J_URI", neo4j_uri)
            neo4j_username = os.environ.get("NEO4J_USERNAME", neo4j_username)
